...
use <- read_excel("withEdSmokPrev.xlsx") #alcohol prevalence
...
write.csv(check,"unimp_smok_prev.csv") #save in case
...
write.csv(df, "imputed_smok_prev.csv")


imputed <-  read.csv("imputed_smok_prev.csv")

imputed_cl <- imputed[,c('smok_prev','gdp','fruit','veggie','meat','temp','Happiness','air_pollution','ed_mean')]
...
add <- read_excel("Pmax200.xlsx", "Tobacco prevalence")
home$yearF <- 1:5 #5 years

df <- data.frame(matrix(ncol = 10, nrow = 8345))
colnames(df)<- c("name", "lower","est","upper","p_val","Pmax","BH_FDR","adjusted_p","r_squared","pearson_cor")


library(nlme)
use<-scale(home$smok_prev, scale = FALSE)
...
for (i in colnames(home[,c(1,5:8348)])){
  ...
  x <- home[[i]]
  home$dum = x
  pos <- grep(i, add$location)
  if (length(pos) != 0){
    df$Pmax[v] <- add$Pmax[pos]
  ...
  if (all(x==0)){
    ...
    fit.arh1<-gls(smok_prev~dum+gdp+fruit+veggie+meat+temp+Happiness+air_pollution+ed_mean,data=home,correlation=corAR1(form=~yearF|country),na.action = na.exclude)
    ...
    df$pearson_cor[v] = cor(home$smok_prev, fit.arh1$fitted)
  }
  v=v+1
}
...
write.csv(df, "fitted_values_for_smok_prev.csv")
